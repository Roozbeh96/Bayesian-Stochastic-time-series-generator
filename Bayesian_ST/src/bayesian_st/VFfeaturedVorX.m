...
%             end
            ...
            for k = 1:size(Obj.HRVFz,1)-1
                numb_prog(k,1) = sum(z_prog>=Obj.HRVFz(k,1)-0.75/2*Obj.HRVFz(k,1)&...
                    z_prog<=Obj.HRVFz(k+1,1)+0.75/2*Obj.HRVFz(k,1),1);
                numb_prog_near(k,1) = sum(z_prog_near>=Obj.HRVFz(k,1)-0.75/2*Obj.HRVFz(k,1)&...
                    z_prog_near<=Obj.HRVFz(k+1,1)+0.75/2*Obj.HRVFz(k,1),1);
                numb_retro(k,1) = sum(z_retro>=Obj.HRVFz(k,1)-0.75/2*Obj.HRVFz(k,1)&...
                    z_retro<=Obj.HRVFz(k+1,1)+0.75/2*Obj.HRVFz(k,1),1);
            end

            zz = (0.75*(Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)));
            figure
            set(gcf,'Position',[941,327,806,463])
            axes('Position',[0.088089330024814,0.138228941684665,0.239454094292804,0.796976241900648])
%             plot(numb_prog./(Obj.HRVFx(end)*size(Obj.HRVFu,3)*Obj.delta),...
%                 (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2*Obj.delta), 'r', 'LineWidth', 2)
%             plot(numb_prog/(Obj.delta),...
%                 (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2*Obj.delta), 'r', 'LineWidth', 2)
            plot(numb_prog./(zz*Obj.HRVFx(end)*size(Obj.HRVFu,3))*(Obj.Delx^2),...
                (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2), 'r', 'LineWidth', 2)
            set(gca,'TickLabelInterpreter','latex','FontSize',15,'XGrid','on',...
                'YGrid','on','XMinorGrid','on','YMinorGrid','on','YScale','linear')
            ylabel('z/$\delta$','Interpreter','Latex','FontSize',15);
            xlabel('\# Prograde vortices/$\delta$','Interpreter','Latex','FontSize',15);

            axes('Position',[0.42059553349876,0.138228941684665,0.239454094292804,0.796976241900648])
%             plot(numb_retro./(Obj.HRVFx(end)*size(Obj.HRVFu,3)*Obj.delta),...
%                 (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2*Obj.delta),'b', 'LineWidth', 2)
%             plot(numb_retro/(Obj.delta),...
%                 (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2*Obj.delta),'b', 'LineWidth', 2)
              plot(numb_retro./(zz*Obj.HRVFx(end)*size(Obj.HRVFu,3))*(Obj.Delx^2),...
                (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2),'b', 'LineWidth', 2)
            set(gca,'TickLabelInterpreter','latex','FontSize',15,'XGrid','on',...
                'YGrid','on','XMinorGrid','on','YMinorGrid','on','YScale','linear')
            ylabel('z/$\delta$','Interpreter','Latex','FontSize',15);
            xlabel('\# Retrograde vortices/$\delta$','Interpreter','Latex','FontSize',15);

            axes('Position',[0.750620347394543,0.138228941684665,0.239454094292804,0.796976241900648])
%             plot(numb_prog_near./(Obj.HRVFx(end)*size(Obj.HRVFu,3)*Obj.delta),...
%                 (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2*Obj.delta), 'k', 'LineWidth', 2)
%             plot(numb_prog_near/(Obj.delta),...
%                 (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2*Obj.delta), 'k', 'LineWidth', 2)
            plot(numb_prog_near./(zz*Obj.HRVFx(end)*size(Obj.HRVFu,3))*(Obj.Delx^2),...
                (Obj.HRVFz(1:end-1) + Obj.HRVFz(2:end)) / (2), 'k', 'LineWidth', 2)
            set(gca,'TickLabelInterpreter','latex','FontSize',15,'XGrid','on',...
                'YGrid','on','XMinorGrid','on','YMinorGrid','on','YScale','linear')
            ylabel('z/$\delta$','Interpreter','Latex','FontSize',15);
            xlabel('\# Prograde near-wall vortices/$\delta$','Interpreter','Latex','FontSize',10);

            disp(['The mean value of vortex distance normalized with ks is: ',...
                num2str(mean(Dist_close_wall_VorX(Dist_close_wall_VorX~=0),2)/(Obj.ks))]);
            disp(['The std value of vortex distance normalized with ks is: ',...
                num2str(std(Dist_close_wall_VorX(Dist_close_wall_VorX~=0),0,2)/(Obj.ks))]);
            if Obj.name == "GenASL"
                Obj.Delx = x_;
            end
        ...
        function Lambda_T_computation(Obj, Aux_Obj)
            if Obj.type == "Exp"
                if Obj.name == "H-WWT7" || Obj.name == "H-WWT10"
                    Obj.Lambda_T = zeros(size(Obj.z, 1),1);
    
                    for i = 1:size(Obj.z, 1)                    
                        Obj.Lambda_T(i) = rms(Obj.uprime(i,:),2)*sqrt(15*Obj.nu/mean([Obj.epsilon_str{i, :}{:}],2));
                    end
                elseif Obj.name == "SonicASL" 
                    Obj.Lambda_T = rms(Obj.uprime(:,1),1)*sqrt(15*Obj.nu/mean([Obj.epsilon_str{1, :}{:}],2));
                elseif Obj.name == "SLPIVASL" 
                    Obj.Lambda_T=sqrt(rms(Obj.uprime(:,26,:),3).^2./...
                    mean((diff(Obj.uprime(:,26,:),1,3)./(mean(Obj.u(8,26,:),3)*1/Obj.fs)).^2,3));
                end
            else
                if Obj.name ~= "GenASL"               
