    ...
    methods
        ...
        function Lambdaci(Obj)
            ...
            if Obj.type == "Exp"
                ...
                for S= 20%1:size(Obj.Lambda_ci,3)
...
%                 Obj.HRVFuprime = Obj.HRVFu-mean(Obj.HRVFu,3);
        ...
        function Gauss_kernel(Obj,kern_xsize, kern_ysize)
            kappa = 0.39;
            mu = [0 0];
%             Sigma = [Obj.Delx/100 0;...
%                 0 Obj.Delx/100];
            Sigma = [Obj.Delx/4000 Obj.Delx/4000*sqrt(tand(12.9));...
                Obj.Delx/4000*sqrt(tand(12.9)) tand(13)*Obj.Delx/4000];
...
%                         u_vor(1:2:end,1:2:end),w_vor(1:2:end,1:2:end)...
...
%                     Obj.HRVFw(indz,indx,S) = Obj.HRVFw(indz,indx,S)+ D2*w_vor;
                ...
                for i=1:Numbvortices
                    N1 = 2;%This should be 2 otherwise E1, E2 has no significant effect
                    E1 = 1;
                    E2 = 1;
                    [Obj,ii,z_prog, z_retro, pr, rt] = lambdacivorX(Obj,i,ii,S,xcs,zcs,rot,dwOlambda_T,...
                        delUwOu_tau, z_prog, z_retro, pr, rt, Trans, N1, E1, E2);
                    xold = xcs(i);
                    N2 = 2;
                    while  xold+N1*dwOlambda_T(ii-1)*Obj.Delx*0.5+N2*dwOlambda_T(ii)*Obj.Delx*0.5<Obj.HRVFx(centroids(i, 5))
                        [Obj,ii,z_prog, z_retro, pr, rt] = lambdacivorX(Obj,i,ii,S,xcs,zcs,rot,dwOlambda_T,...
                        delUwOu_tau, z_prog, z_retro, pr, rt, Trans, N2, E1, E2);
