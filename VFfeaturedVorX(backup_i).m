classdef VFfeaturedVorX(backup) < dynamicprops
    properties
    ...
    methods
        ...
        function Gauss_kernel(Obj,kern_xsize, kern_ysize)
            mu = [0 0];
            Sigma = [Obj.Delx/100 Obj.Delx/100*sqrt(tand(12.9));...
                Obj.Delx/100*sqrt(tand(12.9)) tand(13)*Obj.Delx/100];
...
%             close all
            ...
            for S=1:size(Obj.HRVFu,3)
                ...
                while false%xend_near_wall_prev < max(Obj.HRVFx)
                    
                    N = 1;
                    r_omega = dwOlambda_T(ii)*Obj.Delx*0.5;
                    zc_near_wall = r_omega + Obj.HRVFz(1);
                    xc_near_wall = r_omega + xend_near_wall_prev;
                    xend_near_wall_prev = xc_near_wall + r_omega;
                    [indzmid] = find(Obj.HRVFz>=zc_near_wall,1);
                    [indxmid] = find(Obj.HRVFx>=xc_near_wall,1);
                    if Obj.HRVFw(indzmid,indxmid,S)<0
                       continue 
                    end
...
%                                                 ylim([-2.2 2.2])
                    ...
                    Obj.HRVFu(indz,indx,S) = Obj.HRVFu(indz,indx,S)+ u_vor_trans;
%                     Obj.HRVFuprime(indz,indx,S) = Obj.HRVFuprime(indz,indx,S)+ u_vor_trans;
                    Obj.HRVFw(indz,indx,S) = Obj.HRVFw(indz,indx,S)+ 4*w_vor_trans;
                end                
            end
            Obj.HRVFuprime = Obj.HRVFu - mean(Obj.HRVFu,3);
